...
app.post('/api/server-owner/buy-license',auth,(req,res)=>{const db=req.db,a=req.account,sid=String(req.body?.serverId||''),rank=String(req.body?.rank||'mod');if(!isServerOwner(db,a,sid))return res.status(403).json({error:'Server Owner access required.'});if(!['mod','admin'].includes(rank))return res.status(400).json({error:'Invalid rank.'});const cost=rank==='mod'?25:75;if((a.ownerTokens||0)<cost)return res.status(400).json({error:`You need ${cost} Owner Tokens.`});a.ownerTokens-=cost;a.ownerRankLicenses=a.ownerRankLicenses||{mod:0,admin:0};a.ownerRankLicenses[rank]=(a.ownerRankLicenses[rank]||0)+1;save(db);res.json({ok:true,ownerTokens:a.ownerTokens,licenses:a.ownerRankLicenses})});
