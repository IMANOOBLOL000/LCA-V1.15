...
const DAILY_T=[
 ['messages','Send {n} messages',3,12,15,0,0],['timeEarned','Earn {n} Time',2,10,20,0,0],['pets','Own {n} pets',1,3,25,0,0],['petsBought','Buy {n} pet',1,2,35,0,0],['petsEquipped','Equip {n} pet',1,2,30,0,0],['trades','Complete {n} trade',1,2,45,10,0],['rewardClaims','Claim {n} daily reward',1,1,35,0,1],['pointsEarned','Earn {n} points',25,150,30,5,0],['friends','Have {n} friends',1,4,40,0,0],['achievements','Unlock {n} achievements',1,3,50,10,0]
];
const WEEKLY_T=[
 ['messages','Send {n} messages',15,50,120,25,0],['timeEarned','Earn {n} Time',20,60,180,50,1],['petsBought','Buy {n} pets',2,5,200,50,0],['petsEquipped','Equip {n} pets',3,8,160,25,0],['trades','Complete {n} trades',2,5,250,75,1],['rewardClaims','Claim {n} daily rewards',3,7,200,50,1],['pointsEarned','Earn {n} points',300,1000,220,100,1],['friends','Have {n} friends',3,8,180,50,0],['achievements','Unlock {n} achievements',3,8,300,100,2],['pets','Own {n} pets',5,12,300,100,2]
];
const MONTHLY_T=[
 ['messages','Send {n} messages',60,200,750,250,5],['timeEarned','Earn {n} Time',100,300,900,300,8],['petsBought','Buy {n} pets',5,12,1000,400,5],['petsEquipped','Equip {n} pets',8,20,900,300,5],['trades','Complete {n} trades',5,12,1250,500,10],['rewardClaims','Claim {n} daily rewards',10,20,1000,400,8],['pointsEarned','Earn {n} points',1500,5000,1200,500,10],['friends','Have {n} friends',8,15,900,300,5],['achievements','Unlock {n} achievements',8,20,1500,750,15],['pets','Own {n} pets',12,25,1600,1000,20]
];
function makeChallenges(type,templates){return Array.from({length:100},(_,i)=>{const t=templates[i%templates.length],step=Math.floor(i/templates.length),min=t[2],max=t[3],target=min+Math.floor((max-min)*Math.min(step,9)/9),reward=t[4]+step*25,diamonds=t[5]+step*10,tokens=t[6]+Math.floor(step/3);return{id:`${type}_${i+1}`,name:`${type[0].toUpperCase()+type.slice(1)} Challenge ${i+1}`,description:t[1].replace('{n}',target.toLocaleString()),condition:{type:t[0],target},reward,rewardDiamonds:diamonds,rewardTokens:tokens};});}
...
function featureRandomChallenge(type,a){const list=FEATURE_CHALLENGES[type]||[];const period=periodKey(type);const seed=featureSeed(period+':'+type+':'+a.username);return list[seed%list.length]}
