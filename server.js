...
app.get('/api/time/status',auth,(req,res)=>{const a=req.account,now=Date.now(),last=Number(a.timeLastAt||now);const lastActivity=Number(a.lastActivityAt||now);const active=(now-lastActivity)<=300000;const elapsed=active?Math.max(0,now-last):0;const earned=Math.floor(elapsed/60000);if(earned>0){a.timeBalance=(a.timeBalance||0)+earned;a.timeLastAt=last+earned*60000;save(req.db)}else if(!active&&now-last>60000){a.timeLastAt=now;save(req.db)}res.json({time:a.timeBalance||0,ownerTokens:a.ownerTokens||0,diamonds:a.diamonds||0,minutes:earned,active})});
