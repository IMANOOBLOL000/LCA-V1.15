...
/* ===== LCA FEATURE PACK v1.15-2 ===== */
const FEATURE_RARITIES=[
{id:'common',name:'Common',token:3,diamonds:60,chance:40},{id:'uncommon',name:'Uncommon',token:5,diamonds:100,chance:30},
{id:'rare',name:'Rare',token:10,diamonds:200,chance:20},{id:'epic',name:'Epic',token:25,diamonds:500,chance:6},
{id:'legendary',name:'LEGENDARY',token:50,diamonds:1000,chance:3},{id:'god',name:'GOD',token:250,diamonds:5000,chance:1}];
const FEATURE_PETS=Array.from({length:100},(_,i)=>{const names=['Axolotl','Fox','Cat','Puppy','Bunny','Dragon','Wolf','Penguin','Otter','Shark','Phoenix','Slime','Tiger','Bear','Raccoon','Parrot','Turtle','Panda','Koala','Dino'];const rarity=i<40?'common':i<70?'uncommon':i<90?'rare':i<96?'epic':i<99?'legendary':'god';return{id:'pet_'+String(i+1).padStart(3,'0'),name:names[i%names.length]+' '+(i+1),rarity};});
...
function featureInitAccount(a){a.following=Array.isArray(a.following)?a.following:[];a.followers=Array.isArray(a.followers)?a.followers:[];a.achievements=Array.isArray(a.achievements)?a.achievements:[];a.badges=Array.isArray(a.badges)?a.badges:[];a.challengeClaims=a.challengeClaims||{};a.dailyReward=a.dailyReward||{lastClaim:'',streak:0};a.pets=Array.isArray(a.pets)?a.pets:[];a.petRerollDate=a.petRerollDate||'';a.petShop=Array.isArray(a.petShop)?a.petShop:[];a.level=Number(a.level||1);a.xp=Number(a.xp||0);a.status=String(a.status||'');a.reminders=Array.isArray(a.reminders)?a.reminders:[];a.scheduledMessages=Array.isArray(a.scheduledMessages)?a.scheduledMessages:[]}
function featureDate(){return new Date().toISOString().slice(0,10)} function featureWeek(){const d=new Date(),one=new Date(Date.UTC(d.getUTCFullYear(),0,1));return String(Math.ceil((((d-one)/86400000)+one.getUTCDay()+1)/7))} function featureMonth(){return new Date().toISOString().slice(0,7)}
function featureSeed(str){let n=0;for(let i=0;i<str.length;i++)n=(n*31+str.charCodeAt(i))>>>0;return n}
function weightedRarity(seed,boost=false){
...
function featureDailyPets(a){
  const day=featureDate();
  if(a.petRerollDate!==day||!a.petShop?.length){
    const used=new Set(),ids=[];
    for(let slot=0;slot<5;slot++){
      let rarity=weightedRarity(day+':'+a.username+':slot:'+slot);
      let pool=FEATURE_PETS.filter(x=>x.rarity===rarity&&!used.has(x.id));
      if(!pool.length)pool=FEATURE_PETS.filter(x=>!used.has(x.id));
      if(!pool.length)break;
      const pick=pool[featureSeed(day+':'+a.username+':pick:'+slot)%pool.length];
      used.add(pick.id);ids.push(pick.id);
    }
...
app.post('/api/features/rewards/claim',auth,(req,res)=>{featureInitAccount(req.account);const today=featureDate(),r=req.account.dailyReward;if(r.lastClaim===today)return res.status(400).json({error:'Daily reward already claimed.'});r.streak=Math.min(31,Number(r.streak||0)+1);r.lastClaim=today;const points=25+r.streak*10;req.account.points=Number(req.account.points||0)+points;featureXp(req.account,10);save(req.db);res.json({ok:true,points:req.account.points,streak:r.streak,reward:points})});
...
app.post('/api/features/challenges/claim',auth,(req,res)=>{featureInitAccount(req.account);const type=String(req.body?.type||'');if(!FEATURE_CHALLENGES[type])return res.status(400).json({error:'Invalid challenge type.'});const c=featureRandomChallenge(type,req.account);if(req.account.challengeClaims[c.id])return res.status(400).json({error:'Challenge already claimed.'});req.account.challengeClaims[c.id]=Date.now();req.account.points=Number(req.account.points||0)+c.reward;featureXp(req.account,type==='monthly'?100:50);save(req.db);res.json({ok:true,reward:c.reward,points:req.account.points,level:req.account.level,xp:req.account.xp})});
app.get('/api/features/pets',auth,(req,res)=>{featureInitAccount(req.account);const pets=featureDailyPets(req.account);save(req.db);res.json({pets,owned:req.account.pets,rarities:FEATURE_RARITIES,rollCost:{tokens:100,diamonds:2000},rollBoost:3})});
app.post('/api/features/pets/reroll',auth,(req,res)=>{featureInitAccount(req.account);const costType=String(req.body?.type||'token');const cost=costType==='diamond'?200:10;if(costType==='diamond'){if((req.account.diamonds||0)<cost)return res.status(400).json({error:'Not enough diamonds.'});req.account.diamonds-=cost}else{if((req.account.ownerTokens||0)<cost)return res.status(400).json({error:'Not enough Owner Tokens.'});req.account.ownerTokens-=cost}req.account.petRerollDate='';const pets=featureDailyPets(req.account);save(req.db);res.json({ok:true,pets,diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens})});
app.post('/api/features/pets/buy',auth,(req,res)=>{featureInitAccount(req.account);const id=String(req.body?.id||''),pay=String(req.body?.pay||'token'),pet=FEATURE_PETS.find(x=>x.id===id);if(!pet)return res.status(404).json({error:'Pet not found.'});if(!featureDailyPets(req.account).some(x=>x.id===id))return res.status(400).json({error:"That pet is not in today's shop."});const rr=FEATURE_RARITIES.find(x=>x.id===pet.rarity);if(pay==='diamond'){if((req.account.diamonds||0)<rr.diamonds)return res.status(400).json({error:'Not enough diamonds.'});req.account.diamonds-=rr.diamonds}else{if((req.account.ownerTokens||0)<rr.token)return res.status(400).json({error:'Not enough Owner Tokens.'});req.account.ownerTokens-=rr.token}if(!req.account.pets.includes(id))req.account.pets.push(id);featureXp(req.account,20);save(req.db);res.json({ok:true,pet,owned:req.account.pets,diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens})});
app.post('/api/features/pets/legendary-roll',auth,(req,res)=>{featureInitAccount(req.account);const pay=String(req.body?.pay||'token');if(pay==='diamond'){if((req.account.diamonds||0)<2000)return res.status(400).json({error:'Need 2,000 diamonds.'});req.account.diamonds-=2000}else{if((req.account.ownerTokens||0)<100)return res.status(400).json({error:'Need 100 Owner Tokens.'});req.account.ownerTokens-=100}const rarity=weightedRarity(Date.now()+req.username+':legendary-roll',true);let candidates=FEATURE_PETS.filter(x=>x.rarity===rarity);if(!candidates.length)candidates=FEATURE_PETS.filter(x=>x.rarity==='legendary'||x.rarity==='god');const pet=candidates[featureSeed(String(Date.now())+req.username)%candidates.length];if(!req.account.pets.includes(pet.id))req.account.pets.push(pet.id);featureXp(req.account,100);save(req.db);res.json({ok:true,pet,boost:'3 percentage points to the combined Legendary/GOD chance',diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens,owned:req.account.pets})});
app.get('/api/features/trading',auth,(req,res)=>{const db=req.db;db.featureTrades=Array.isArray(db.featureTrades)?db.featureTrades:[];res.json({listings:db.featureTrades.filter(x=>x.status==='listed').slice(-100)})});
app.post('/api/features/trading/list',auth,(req,res)=>{featureInitAccount(req.account);const db=req.db;db.featureTrades=Array.isArray(db.featureTrades)?db.featureTrades:[];const pet=FEATURE_PETS.find(x=>x.id===String(req.body?.petId||''));if(!pet||!req.account.pets.includes(pet.id))return res.status(400).json({error:'You do not own that pet.'});const price=Math.max(1,Math.floor(Number(req.body?.price||0)));if(!price)return res.status(400).json({error:'Enter a price.'});db.featureTrades.push({id:'trade_'+Date.now().toString(36),seller:req.username,petId:pet.id,price,status:'listed',createdAt:Date.now()});save(db);res.json({ok:true})});
app.post('/api/features/trading/bid',auth,(req,res)=>{const db=req.db;db.featureTrades=Array.isArray(db.featureTrades)?db.featureTrades:[];const id=String(req.body?.id||''),bid=Math.floor(Number(req.body?.bid||0)),l=db.featureTrades.find(x=>x.id===id&&x.status==='listed');if(!l)return res.status(404).json({error:'Listing not found.'});if(l.seller===req.username)return res.status(400).json({error:'You cannot bid on your own listing.'});if(bid<l.price||Number(req.account.diamonds||0)<bid)return res.status(400).json({error:'Bid must meet the asking price and your diamond balance.'});req.account.diamonds-=bid;const seller=db.accounts[l.seller];seller.diamonds=Number(seller.diamonds||0)+bid;l.status='sold';l.buyer=req.username;l.bid=bid;save(db);res.json({ok:true,diamonds:req.account.diamonds})});
app.post('/api/features/trading/trade',auth,(req,res)=>{featureInitAccount(req.account);const db=req.db,target=String(req.body?.target||'').trim().toLowerCase(),offer=String(req.body?.offerPetId||''),want=String(req.body?.wantPetId||'');const other=db.accounts[target];if(!other||target===req.username)return res.status(400).json({error:'Choose another valid player.'});featureInitAccount(other);if(!req.account.pets.includes(offer))return res.status(400).json({error:'You do not own the pet you are offering.'});if(!other.pets.includes(want))return res.status(400).json({error:'That player does not own the requested pet.'});req.account.pets=req.account.pets.filter(x=>x!==offer);other.pets=other.pets.filter(x=>x!==want);req.account.pets.push(want);other.pets.push(offer);featureXp(req.account,50);featureXp(other,50);save(db);res.json({ok:true,message:`Traded ${offer} for ${want}.`,owned:req.account.pets})});
