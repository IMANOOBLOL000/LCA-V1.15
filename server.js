...
function auth(req,res,next){const t=(req.headers.authorization||'').replace(/^Bearer\s+/,'');const db=ensureDb();const s=db.sessions[t];if(!s||!db.accounts[s.username])return res.status(401).json({error:'Please log in again.'});const a=db.accounts[s.username];if(a.permanentBan|| (a.banUntil&&Number(a.banUntil)>Date.now())){delete db.sessions[t];save(db);return res.status(403).json({error:'You are banned from LCA.',banned:true,banUntil:Number(a.banUntil),banReason:a.banReason||''})}if(!a.permanentBan&&a.banUntil&&Number(a.banUntil)<=Date.now()){a.banUntil=0;a.banReason='';save(db)}s.lastUsed=Date.now();req.token=t;req.username=s.username;req.db=db;req.account=a;next()}
