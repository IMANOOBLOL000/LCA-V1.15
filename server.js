...
app.post('/api/poll',(req,res)=>{
  ...
  if(!srv)return res.status(404).json({error:'Server not found.'});
  const role=effectiveRole(a);
  if(!['owner','co-owner','server-admin'].includes(role))return res.status(403).json({error:'Only Server Admin, Co-Owner, or Owner can create polls.'});
