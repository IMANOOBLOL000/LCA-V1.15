...
/* ===== LCA FEATURE PACK v1.21 PET/TRADING/CHALLENGE UPGRADE ===== */
...
'Rabbit','Fox','Cat','Puppy','Bunny','Dragon','Wolf','Penguin','Otter','Shark','Phoenix','Slime','Tiger','Bear','Raccoon','Parrot','Turtle','Panda','Koala','Dino','Frog','Hedgehog','Deer','Monkey','Hamster','Duck','Owl','Goat','Cow','Pig','Horse','Zebra','Giraffe','Lion','Cheetah','Leopard','Crocodile','Alligator','Seal','Walrus','Whale','Dolphin','Octopus','Jellyfish','Crab','Lobster','Starfish','Seahorse','Swordfish','Manta','Falcon','Eagle','Hawk','Raven','Swan','Peacock','Flamingo','Toucan','Bee','Butterfly','Ladybug','Beetle','Firefly','Ant','Snail','Scorpion','Spider','Moth','Dragonfly','Griffin','Unicorn','Pegasus','Golem','Goblin','Wizard','Knight','Robot','Alien','Astronaut','Meteor','Comet','Galaxy','Nebula','Crystal','Shadow','Storm','Volcano','Frost','Solar','Lunar','Void','Cosmic','Titan','Serpent','Hydra','Kraken','Phoenix Prime','Celestial','Eclipse','Infinity'
];
const PET_EMOJIS=['🐇','🦊','🐱','🐶','🐰','🐉','🐺','🐧','🦦','🦈','🔥','🟢','🐯','🐻','🦝','🦜','🐢','🐼','🐨','🦖','🐸','🦔','🦌','🐒','🐹','🦆','🦉','🐐','🐮','🐷','🐴','🦓','🦒','🦁','🐆','🐊','🐊','🦭','🦭','🐋','🐬','🐙','🪼','🦀','🦞','⭐','🐚','🐴','🐟','🦅','🦅','🦅','🐦','🦢','🦚','🦩','🦜','🐝','🦋','🐞','🪲','✨','🐜','🐌','🦂','🕷️','🦋','🐲','🦄','🪽','🗿','👺','🧙','🛡️','🤖','👽','👨‍🚀','☄️','☄️','🌌','💎','🌑','🌩️','🌋','❄️','☀️','🌙','🕳️','🌌','🗿','🐍','🐲','🐙','🔥','🌠','🌌','♾️'];
const PET_RARITY_BY_INDEX=i=>i<40?'common':i<70?'uncommon':i<90?'rare':i<96?'epic':i<99?'legendary':'god';
const FEATURE_PETS=PET_BASE_NAMES.map((base,i)=>{const rarity=PET_RARITY_BY_INDEX(i),rr=FEATURE_RARITIES.find(x=>x.id===rarity);return{id:`${base.toLowerCase().replace(/[^a-z0-9]+/g,'-')}-${rarity}`,name:`${base}-${rarity}`,displayName:base,rarity,value:rr.value,pointMult:rr.pointMult,diamondMult:rr.diamondMult,tokenMult:rr.tokenMult,emoji:PET_EMOJIS[i]||'🐾'};});

const ACH_CATS=['Chatting','Social','Servers','Economy','Pets','Challenges','Exploration','Collection','Milestones','Progress'];
const ACH_TYPES=[
  ['messages','messages',1,'Send'],['friends','friends',1,'Make'],['servers','servers',1,'Join/create'],['points','points',100,'Earn'],['pets','pets',1,'Own'],['challengeCompleted','challengeCompleted',1,'Complete'],['backgrounds','backgrounds',1,'Collect'],['pets','pets',1,'Collect'],['level','level',2,'Reach'],['timeEarned','timeEarned',5,'Earn']
];
const FEATURE_ACHIEVEMENTS=Array.from({length:100},(_,i)=>{const k=Math.floor(i/10),step=(i%10)+1,[label,key,base,verb]=ACH_TYPES[k],target=key==='points'?base*step:key==='level'?base+step-1:base*step;return{id:'ach_'+(i+1),name:`${ACH_CATS[k]} Achievement ${i+1}`,category:ACH_CATS[k],description:`${verb} ${target.toLocaleString()} ${label}.`,condition:{type:key,target},rewardBadge:`badge_${i+1}`};});

const DAILY_T=[
 ['messages','Send {n} messages',3,12],['timeEarned','Earn {n} Time',2,10],['pets','Own {n} pets',1,3],['petsBought','Buy {n} pet',1,2],['petsEquipped','Equip {n} pet',1,2],['trades','Complete {n} trade',1,2],['rewardClaims','Claim {n} daily reward',1,1],['pointsEarned','Earn {n} points',25,150],['friends','Have {n} friends',1,4],['achievements','Unlock {n} achievements',1,3]
];
const WEEKLY_T=[
 ['messages','Send {n} messages',15,50],['timeEarned','Earn {n} Time',20,60],['petsBought','Buy {n} pets',2,5],['petsEquipped','Equip {n} pets',3,8],['trades','Complete {n} trades',2,5],['rewardClaims','Claim {n} daily rewards',3,7],['pointsEarned','Earn {n} points',300,1000],['friends','Have {n} friends',3,8],['achievements','Unlock {n} achievements',3,8],['pets','Own {n} pets',5,12]
];
const MONTHLY_T=[
 ['messages','Send {n} messages',60,200],['timeEarned','Earn {n} Time',100,300],['petsBought','Buy {n} pets',5,12],['petsEquipped','Equip {n} pets',8,20],['trades','Complete {n} trades',5,12],['rewardClaims','Claim {n} daily rewards',10,20],['pointsEarned','Earn {n} points',1500,5000],['friends','Have {n} friends',8,15],['achievements','Unlock {n} achievements',8,20],['pets','Own {n} pets',12,25]
];
function makeChallenges(type,templates){return Array.from({length:100},(_,i)=>{const t=templates[i%templates.length],step=Math.floor(i/templates.length),min=t[2],max=t[3],target=min+Math.floor((max-min)*Math.min(step,9)/9);let reward,rewardDiamonds,rewardTokens;if(type==='daily'){reward=25+step*5;rewardDiamonds=step>=2?Math.min(5,Math.floor(step/2)):0;rewardTokens=step>=8?1:0;}else if(type==='weekly'){reward=125+step*20;rewardDiamonds=10+step*3;rewardTokens=step>=4?Math.min(2,Math.floor(step/4)):0;}else{reward=500+step*110;rewardDiamonds=50+step*15;rewardTokens=2+Math.floor(step*6/9);}return{id:`${type}_${i+1}`,name:`${type[0].toUpperCase()+type.slice(1)} Challenge ${i+1}`,description:t[1].replace('{n}',target.toLocaleString()),condition:{type:t[0],target},reward,rewardDiamonds,rewardTokens};});}
const FEATURE_CHALLENGES={daily:makeChallenges('daily',DAILY_T),weekly:makeChallenges('weekly',WEEKLY_T),monthly:makeChallenges('monthly',MONTHLY_T)};
function periodKey(type){if(type==='daily')return featureDate();if(type==='weekly')return `${new Date().getUTCFullYear()}-W${featureWeek()}`;return featureMonth();}
function featureInitAccount(a){
  a.following=Array.isArray(a.following)?a.following:[];a.followers=Array.isArray(a.followers)?a.followers:[];a.achievements=Array.isArray(a.achievements)?a.achievements:[];a.badges=Array.isArray(a.badges)?a.badges:[];
  a.challengeClaims=a.challengeClaims||{};a.dailyReward=a.dailyReward||{lastClaim:'',streak:0};a.featureProgress=a.featureProgress||{};a.featureLifetime=a.featureLifetime||{};a.challengeCompleted=Number(a.challengeCompleted||0);
  a.challengeReroll=a.challengeReroll||{period:'',used:false,seed:0};const rerollPeriod=featureDate();if(a.challengeReroll.period!==rerollPeriod){a.challengeReroll={period:rerollPeriod,used:false,seed:0};}
  a.pets=Array.isArray(a.pets)?a.pets:[];a.equippedPets=Array.isArray(a.equippedPets)?a.equippedPets:[];a.petRerollDate=a.petRerollDate||'';a.petShop=Array.isArray(a.petShop)?a.petShop:[];a.petMarketSeed=a.petMarketSeed||crypto.randomBytes(8).toString('hex');
  ...
  for(const type of ['daily','weekly','monthly']){const key=periodKey(type);if(a.featureProgress[type]?.period!==key)a.featureProgress[type]={period:key,counts:{}};}
}
function featureDate(){return new Date().toISOString().slice(0,10)}
function featureWeek(){const d=new Date(),one=new Date(Date.UTC(d.getUTCFullYear(),0,1));return String(Math.ceil((((d-one)/86400000)+one.getUTCDay()+1)/7)).padStart(2,'0')}
function featureMonth(){return new Date().toISOString().slice(0,7)}
function bumpFeature(a,key,n=1){featureInitAccount(a);for(const type of ['daily','weekly','monthly'])a.featureProgress[type].counts[key]=Number(a.featureProgress[type].counts[key]||0)+Number(n||0);}
function featureProgress(a,c,type){featureInitAccount(a);const k=c.condition?.type,target=Number(c.condition?.target||1);let value=0;
  if(k==='friends')value=(a.friends||[]).length;else if(k==='servers')value=(a.servers||[]).length;else if(k==='points')value=Number(a.points||0);else if(k==='pets')value=(a.pets||[]).length;else if(k==='achievements')value=(a.achievements||[]).length;else if(k==='backgrounds')value=(a.inventory||[]).filter(x=>Object.prototype.hasOwnProperty.call(SHOP_ITEMS,x)).length;else if(k==='level')value=Number(a.level||1);else value=Number(a.featureProgress?.[type]?.counts?.[k]||0);
...
function weightedRarityRandom(boost=false){const weights={common:40,uncommon:30,rare:20,epic:6,legendary:3,god:1};if(boost){weights.legendary+=1.5;weights.god+=1.5;weights.common-=1;weights.uncommon-=1;weights.rare-=1;}const total=Object.values(weights).reduce((a,b)=>a+b,0);let n=Math.random()*total;for(const [rar,w] of Object.entries(weights)){n-=w;if(n<=0)return rar}return 'common';}
function featureDistinctShop(a){
  featureInitAccount(a);const day=featureDate();
  if(a.petRerollDate!==day||!a.petShop?.length){const ids=[],used=new Set();for(let slot=0;slot<5;slot++){let rarity=weightedRarityRandom(false),pool=FEATURE_PETS.filter(x=>x.rarity===rarity&&!used.has(x.id));if(!pool.length)pool=FEATURE_PETS.filter(x=>!used.has(x.id));const pick=pool[Math.floor(Math.random()*pool.length)];used.add(pick.id);ids.push(pick.id);}a.petShop=ids;a.petRerollDate=day;a.petMarketSeed=crypto.randomBytes(8).toString('hex');}
  return a.petShop.map(id=>FEATURE_PETS.find(x=>x.id===id)).filter(Boolean);
}
function featureRandomChallenge(type,a){const list=FEATURE_CHALLENGES[type]||[];const period=periodKey(type);const rerollSeed=Number(a.challengeReroll?.seed||0);const seed=featureSeed(period+':'+type+':'+a.username+':reroll:'+rerollSeed);return list[seed%list.length]}
function challengeSetStatus(a){featureInitAccount(a);const out={};for(const type of ['daily','weekly','monthly']){const c=featureRandomChallenge(type,a),p=featureProgress(a,c,type),claimKey=`${periodKey(type)}:${c.id}`;out[type]={complete:p.complete,claimed:!!a.challengeClaims[claimKey]};}return out;}
function featureXp(a,amount){a.xp=Number(a.xp||0)+Math.max(0,Number(amount||0));a.level=Math.max(1,Math.floor(a.xp/100)+1)}
function achievementProgress(a,x){const k=x.condition.type,t=Number(x.condition.target||1);let v=0;if(k==='friends')v=(a.friends||[]).length;else if(k==='servers')v=(a.servers||[]).length;else if(k==='points')v=Number(a.points||0);else if(k==='pets')v=(a.pets||[]).length;else if(k==='challengeCompleted')v=Number(a.challengeCompleted||0);else if(k==='backgrounds')v=(a.inventory||[]).filter(id=>Object.prototype.hasOwnProperty.call(SHOP_ITEMS,id)).length;else if(k==='achievements')v=(a.achievements||[]).length;else if(k==='level')v=Number(a.level||1);else v=Number(a.featureLifetime?.[k]||0);return {value:v,target:t,complete:v>=t};}
function petStats(a){const equipped=(a.equippedPets||[]).map(id=>FEATURE_PETS.find(p=>p.id===id)).filter(Boolean);let point=1,diamond=1,token=1;for(const p of equipped){point*=p.pointMult;diamond*=p.diamondMult;token*=p.tokenMult}return{point:Math.round(point*100)/100,diamond:Math.round(diamond*100)/100,token:Math.round(token*100)/100,equipped};}
...
app.get('/api/features/achievements',auth,(req,res)=>{featureInitAccount(req.account);res.json({achievements:FEATURE_ACHIEVEMENTS.map(x=>({...x,progress:achievementProgress(req.account,x),unlocked:req.account.achievements.includes(x.id)})),badges:req.account.badges||[]})});
app.post('/api/features/achievements/unlock',auth,(req,res)=>{featureInitAccount(req.account);const x=FEATURE_ACHIEVEMENTS.find(v=>v.id===String(req.body?.id||''));if(!x)return res.status(404).json({error:'Achievement not found.'});const p=achievementProgress(req.account,x);if(!p.complete)return res.status(400).json({error:`Not complete yet: ${x.description}`});if(req.account.achievements.includes(x.id))return res.status(400).json({error:'That badge is already claimed.'});req.account.achievements.push(x.id);req.account.badges.push(x.rewardBadge);featureXp(req.account,25);save(req.db);res.json({ok:true,achievement:x,level:req.account.level,xp:req.account.xp})});
...
app.get('/api/features/rewards',auth,(req,res)=>{featureInitAccount(req.account);const today=featureDate(),r=req.account.dailyReward;const now=Date.now(),next=new Date();next.setUTCHours(24,0,0,0);res.json({today,lastClaim:r.lastClaim,streak:r.streak,canClaim:r.lastClaim!==today,nextRewardAt:next.getTime(),calendar:Array.from({length:31},(_,i)=>({day:i+1,claimed:i<r.streak,available:i+1===Math.min(r.streak+1,31)}))})});
app.post('/api/features/rewards/claim',auth,(req,res)=>{featureInitAccount(req.account);const today=featureDate(),r=req.account.dailyReward;if(r.lastClaim===today)return res.status(400).json({error:'Daily reward already claimed.'});r.streak=Math.min(31,Number(r.streak||0)+1);r.lastClaim=today;const points=25+r.streak*10;req.account.points=Number(req.account.points||0)+Math.floor(points*petStats(req.account).point);bumpFeature(req.account,'rewardClaims',1);featureXp(req.account,10);save(req.db);res.json({ok:true,points:req.account.points,streak:r.streak,reward:points})});
app.get('/api/features/challenges',auth,(req,res)=>{featureInitAccount(req.account);const out={};for(const type of ['daily','weekly','monthly']){const c=featureRandomChallenge(type,req.account),p=featureProgress(req.account,c,type),claimKey=`${periodKey(type)}:${c.id}`;out[type]={...c,progress:p,claimed:!!req.account.challengeClaims[claimKey]}}const status=challengeSetStatus(req.account);const allClaimed=Object.values(status).every(x=>x.claimed);res.json({catalogCounts:{daily:100,weekly:100,monthly:100},current:out,reroll:{available:allClaimed&&!req.account.challengeReroll.used,used:!!req.account.challengeReroll.used}})});
app.post('/api/features/challenges/claim',auth,(req,res)=>{featureInitAccount(req.account);const type=String(req.body?.type||'');if(!FEATURE_CHALLENGES[type])return res.status(400).json({error:'Invalid challenge type.'});const c=featureRandomChallenge(type,req.account),p=featureProgress(req.account,c,type),claimKey=`${periodKey(type)}:${c.id}`;if(!p.complete)return res.status(400).json({error:`Challenge incomplete: ${c.description} (${p.value}/${p.target})`});if(req.account.challengeClaims[claimKey])return res.status(400).json({error:'Challenge reward already claimed for this challenge.'});req.account.challengeClaims[claimKey]=Date.now();req.account.points=Number(req.account.points||0)+Math.floor(c.reward*petStats(req.account).point);req.account.diamonds=Number(req.account.diamonds||0)+Number(c.rewardDiamonds||0);req.account.ownerTokens=Number(req.account.ownerTokens||0)+Number(c.rewardTokens||0);req.account.challengeCompleted=Number(req.account.challengeCompleted||0)+1;req.account.featureLifetime=req.account.featureLifetime||{};req.account.featureLifetime.challengeCompleted=Number(req.account.featureLifetime.challengeCompleted||0)+1;featureXp(req.account,type==='monthly'?150:type==='weekly'?75:40);save(req.db);const status=challengeSetStatus(req.account),allClaimed=Object.values(status).every(x=>x.claimed);res.json({ok:true,reward:c.reward,rewardDiamonds:c.rewardDiamonds,rewardTokens:c.rewardTokens,points:req.account.points,diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens,level:req.account.level,xp:req.account.xp,rerollAvailable:allClaimed&&!req.account.challengeReroll.used})});
app.post('/api/features/challenges/reroll',auth,(req,res)=>{featureInitAccount(req.account);if(req.account.challengeReroll.used)return res.status(400).json({error:'You already used this challenge reroll for today.'});const status=challengeSetStatus(req.account);if(!Object.values(status).every(x=>x.claimed))return res.status(400).json({error:'Complete and claim all 3 challenges before rerolling.'});req.account.challengeReroll.seed=Number(req.account.challengeReroll.seed||0)+1;req.account.challengeReroll.used=true;for(const type of ['daily','weekly','monthly'])req.account.featureProgress[type]={period:periodKey(type),counts:{}};save(req.db);const out={};for(const type of ['daily','weekly','monthly']){const c=featureRandomChallenge(type,req.account);out[type]=c;}res.json({ok:true,current:out,reroll:{available:false,used:true}})});
app.get('/api/features/pets',auth,(req,res)=>{featureInitAccount(req.account);const pets=featureDistinctShop(req.account),stats=petStats(req.account);save(req.db);res.json({pets,owned:req.account.pets,rarities:FEATURE_RARITIES,stats,equipped:req.account.equippedPets,rollCost:{tokens:10,diamonds:200},rollCostDaily:{tokens:10,diamonds:200},rollCostLegendary:{tokens:100,diamonds:2000},rollBoost:3,guide:FEATURE_PETS})});
app.post('/api/features/pets/reroll',auth,(req,res)=>{featureInitAccount(req.account);const costType=String(req.body?.type||'token');const cost=costType==='diamond'?200:10;if(costType==='diamond'){if((req.account.diamonds||0)<cost)return res.status(400).json({error:'Not enough diamonds.'});req.account.diamonds-=cost}else{if((req.account.ownerTokens||0)<cost)return res.status(400).json({error:'Not enough Owner Tokens.'});req.account.ownerTokens-=cost}req.account.petRerollDate='';req.account.petShop=[];const pets=featureDistinctShop(req.account);save(req.db);res.json({ok:true,pets,diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens});});
app.post('/api/features/pets/buy',auth,(req,res)=>{featureInitAccount(req.account);const id=String(req.body?.id||''),pay=String(req.body?.pay||'token'),pet=FEATURE_PETS.find(x=>x.id===id);if(!pet)return res.status(404).json({error:'Pet not found.'});if(req.account.pets.includes(id))return res.status(400).json({error:'You can only own each pet once.'});const shop=featureDistinctShop(req.account);if(!shop.some(x=>x.id===id))return res.status(400).json({error:"That pet is not in today's shop."});const rr=FEATURE_RARITIES.find(x=>x.id===pet.rarity);if(pay==='diamond'){if((req.account.diamonds||0)<rr.diamonds)return res.status(400).json({error:'Not enough diamonds.'});req.account.diamonds-=rr.diamonds}else{if((req.account.ownerTokens||0)<rr.token)return res.status(400).json({error:'Not enough Owner Tokens.'});req.account.ownerTokens-=rr.token}req.account.pets.push(id);bumpFeature(req.account,'petsBought',1);featureXp(req.account,20);save(req.db);res.json({ok:true,pet,owned:req.account.pets,diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens,stats:petStats(req.account)});});
app.post('/api/features/pets/legendary-roll',auth,(req,res)=>{featureInitAccount(req.account);const pay=String(req.body?.pay||'token');if(pay==='diamond'){if((req.account.diamonds||0)<2000)return res.status(400).json({error:'Need 2,000 diamonds.'});req.account.diamonds-=2000}else{if((req.account.ownerTokens||0)<100)return res.status(400).json({error:'Need 100 Owner Tokens.'});req.account.ownerTokens-=100}const candidates=FEATURE_PETS.filter(x=>['legendary','god'].includes(x.rarity)&&!req.account.pets.includes(x.id));if(!candidates.length)return res.status(400).json({error:'You already own every LEGENDARY and GOD pet.'});const god=Math.random()<0.25, pool=candidates.filter(x=>x.rarity===(god?'god':'legendary'));const pet=(pool.length?pool:candidates)[Math.floor(Math.random()*(pool.length?pool:candidates).length)];req.account.pets.push(pet.id);bumpFeature(req.account,'petsBought',1);featureXp(req.account,100);save(req.db);res.json({ok:true,pet,boost:'Legendary/GOD roll',diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens,owned:req.account.pets,stats:petStats(req.account)});});
...
app.post('/api/features/pets/equip',auth,(req,res)=>{featureInitAccount(req.account);const id=String(req.body?.id||''),pet=FEATURE_PETS.find(x=>x.id===id);if(!pet||!req.account.pets.includes(id))return res.status(400).json({error:'You do not own that pet.'});if(req.account.equippedPets.includes(id))return res.status(400).json({error:'That pet is already equipped.'});if(req.account.equippedPets.length>=3)return res.status(400).json({error:'You can equip up to 3 pets.'});req.account.equippedPets.push(id);bumpFeature(req.account,'petsEquipped',1);save(req.db);res.json({ok:true,equipped:req.account.equippedPets,stats:petStats(req.account)});});
app.post('/api/features/pets/unequip',auth,(req,res)=>{featureInitAccount(req.account);const id=String(req.body?.id||'');if(!req.account.equippedPets.includes(id))return res.status(400).json({error:'That pet is not equipped.'});req.account.equippedPets=req.account.equippedPets.filter(x=>x!==id);save(req.db);res.json({ok:true,equipped:req.account.equippedPets,stats:petStats(req.account)});});
...
app.get('/api/features/trading',auth,(req,res)=>{featureInitAccount(req.account);const db=req.db;ensureTradeStore(db);const online=onlineUsers(db),players=Object.values(db.accounts).filter(a=>a.username!==req.username).map(a=>({username:a.username,name:a.name,online:online.has(a.username),pets:(a.pets||[]).length})).sort((a,b)=>Number(b.online)-Number(a.online)||String(a.name).localeCompare(String(b.name)));const pending=db.featureTradeRequests.filter(x=>x.to===req.username&&x.status==='pending').map(x=>cleanTrade(db,x));const active=db.featureTradeRequests.filter(x=>x.status==='active'&&(x.from===req.username||x.to===req.username)).map(x=>cleanTrade(db,x));const listings=db.featureTrades.filter(x=>x.status==='listed').slice(-100).map(x=>({...x,pet:FEATURE_PETS.find(p=>p.id===x.petId),value:FEATURE_PETS.find(p=>p.id===x.petId)?.value||0}));res.json({players,listings,pending,active,owned:req.account.pets,values:FEATURE_PETS.map(p=>({id:p.id,name:p.name,value:p.value,rarity:p.rarity,emoji:p.emoji}))});});
app.post('/api/features/trading/request',auth,(req,res)=>{featureInitAccount(req.account);const db=req.db;ensureTradeStore(db);const target=String(req.body?.target||'').trim().toLowerCase();if(!db.accounts[target]||target===req.username)return res.status(400).json({error:'Choose another valid player.'});if(!onlineUsers(db).has(target))return res.status(400).json({error:'That player is offline. Trade requests can only be sent to online players.'});const existing=db.featureTradeRequests.find(x=>x.status==='pending'&&x.from===req.username&&x.to===target);if(existing)return res.status(400).json({error:'Trade request already sent.'});const t={id:'trade_'+crypto.randomBytes(8).toString('hex'),from:req.username,to:target,offer:[],request:[],fromAccepted:false,toAccepted:false,status:'pending',createdAt:Date.now()};db.featureTradeRequests.push(t);save(db);res.json({ok:true,trade:cleanTrade(db,t)});});
