...
app.get('/api/time/status',auth,(req,res)=>{const a=req.account,now=Date.now();const last=Number(a.timeLastAt||now);const lastActivity=Number(a.lastActivityAt||now);const active=(now-lastActivity)<=300000;const elapsed=active?Math.max(0,now-last):0;const earned=Math.floor(elapsed/60000);if(earned>0){a.timeBalance=Number(a.timeBalance||0)+earned;a.timeLastAt=last+earned*60000;save(req.db)}res.json({time:Number(a.timeBalance||0),ownerTokens:Number(a.ownerTokens||0),diamonds:Number(a.diamonds||0),minutes:earned,active,rate:'1 Time per active minute'});});
...
function featureDailyPets(a){const day=featureDate();if(a.petRerollDate!==day||!a.petShop?.length){const seed=featureSeed(day+':'+a.username);let arr=FEATURE_PETS.slice().sort((x,y)=>featureSeed(x.id+seed)-featureSeed(y.id+seed));a.petShop=arr.slice(0,5).map(x=>x.id);a.petRerollDate=day}return a.petShop.map(id=>FEATURE_PETS.find(x=>x.id===id)).filter(Boolean)}
...
app.post('/api/features/pets/legendary-roll',auth,(req,res)=>{featureInitAccount(req.account);const pay=String(req.body?.pay||'token');if(pay==='diamond'){if((req.account.diamonds||0)<2000)return res.status(400).json({error:'Need 2,000 diamonds.'});req.account.diamonds-=2000}else{if((req.account.ownerTokens||0)<100)return res.status(400).json({error:'Need 100 Owner Tokens.'});req.account.ownerTokens-=100}const boosted=Math.random()<0.12;const candidates=FEATURE_PETS.filter(x=>x.rarity==='legendary'||x.rarity==='god');const normal=FEATURE_PETS[Math.floor(Math.random()*FEATURE_PETS.length)];const pet=boosted?candidates[Math.floor(Math.random()*candidates.length)]:normal;if(!req.account.pets.includes(pet.id))req.account.pets.push(pet.id);featureXp(req.account,100);save(req.db);res.json({ok:true,pet,boost:3,diamonds:req.account.diamonds,ownerTokens:req.account.ownerTokens,owned:req.account.pets})});
