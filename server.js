...
const DATA=process.env.LCA_DATA||path.join(__dirname,'.lca-db.json');
...
const ROLE_LIMITS={
  mod:{announcement:{limit:3,window:'day'},moderation:{limit:10,window:'day'},removeMember:{limit:5,window:'day'}},
  admin:{announcement:{limit:5,window:'day'},moderation:{limit:20,window:'day'},removeMember:{limit:10,window:'day'}},
  'server-admin':{announcement:{limit:4,window:'hour'},moderation:{limit:10,window:'hour'},removeMember:{limit:5,window:'hour'}}
};
function periodKey(window){return window==='hour'?new Date().toISOString().slice(0,13):new Date().toISOString().slice(0,10)}
function consumeStaff(a,kind){const role=effectiveRole(a),cfg=ROLE_LIMITS[role]?.[kind];if(!cfg)return {ok:false,error:'Staff access only.'};const key=periodKey(cfg.window);const dateKey=kind==='announcement'?'staffAnnouncementDate':'staffActionDate';const countKey=kind==='announcement'?'staffAnnouncementCount':'staffActionCount';if(a[dateKey]!==key){a[dateKey]=key;a[countKey]=0}if(Number(a[countKey]||0)>=cfg.limit)return {ok:false,error:`You have used all ${cfg.limit} ${cfg.window==='hour'?'hourly':'daily'} ${kind} actions.`};a[countKey]=Number(a[countKey]||0)+1;return {ok:true}}
function staffCanServer(db,a,serverId){if(!a||!db.servers[serverId])return false;if(isRealOwner(a))return true;const srv=db.servers[serverId],role=effectiveRole(a);if(!srv.members.includes(a.username))return false;if(role==='server-admin')return a.serverAdminServerId===serverId&&Number(a.serverAdminExpiresAt||0)>Date.now();if(role==='server-owner')return String(srv.owner||'').toLowerCase()===String(a.username||'').toLowerCase();return ['mod','admin'].includes(role)}
...
app.get('/api/time/status',auth,(req,res)=>{const a=req.account,now=Date.now(),last=Number(a.timeLastAt||now);const active=Number(req.previousSessionLastUsed||now)>=now-120000;const elapsed=active?Math.max(0,now-last):0;const earned=Math.min(5,Math.floor(elapsed/60000));if(earned){a.timeBalance=(a.timeBalance||0)+earned;a.timeLastAt=last+earned*60000;save(req.db)}else if(!active){a.timeLastAt=now;save(req.db)}res.json({time:a.timeBalance||0,ownerTokens:a.ownerTokens||0,diamonds:a.diamonds||0,minutes:earned})});
app.post('/api/time/exchange',auth,(req,res)=>{const a=req.account,type=String(req.body?.type||'');let need=0,give=0;if(type==='token'){need=10;give=1}else if(type==='diamond'){need=5;give=1}else return res.status(400).json({error:'Unknown exchange.'});if((a.timeBalance||0)<need)return res.status(400).json({error:`You need ${need} time.`});a.timeBalance-=need;if(type==='token')a.ownerTokens=(a.ownerTokens||0)+give;else a.diamonds=(a.diamonds||0)+give;save(req.db);res.json({ok:true,time:a.timeBalance,ownerTokens:a.ownerTokens||0,diamonds:a.diamonds||0})});
app.post('/api/owner/time',ownerControl,(req,res)=>{if(!isRealOwner(req.account))return res.status(403).json({error:'Only the Owner can give Time.'});const db=req.db,target=String(req.body?.target||req.username).trim().toLowerCase(),amount=Math.floor(Number(req.body?.amount||0));if(!db.accounts[target])return res.status(404).json({error:'Player not found.'});if(!Number.isInteger(amount)||amount<=0)return res.status(400).json({error:'Enter a positive whole amount.'});db.accounts[target].timeBalance=(db.accounts[target].timeBalance||0)+amount;save(db);res.json({ok:true,time:db.accounts[target].timeBalance,target,account:clean(db.accounts[target])})});
app.post('/api/owner/tokens',ownerControl,(req,res)=>{if(!isRealOwner(req.account))return res.status(403).json({error:'Only the Owner can give Owner Tokens.'});const db=req.db,target=String(req.body?.target||'').toLowerCase(),amount=Math.floor(Number(req.body?.amount||0));if(!db.accounts[target])return res.status(404).json({error:'Player not found.'});if(!Number.isFinite(amount)||amount<=0)return res.status(400).json({error:'Enter a positive amount.'});db.accounts[target].ownerTokens=(db.accounts[target].ownerTokens||0)+amount;save(db);res.json({ok:true,ownerTokens:db.accounts[target].ownerTokens})});
...
app.post('/api/server-owner/buy-license',auth,(req,res)=>{const db=req.db,a=req.account,sid=String(req.body?.serverId||''),rank=String(req.body?.rank||'mod');if(!isServerOwner(db,a,sid))return res.status(403).json({error:'Server Owner access required.'});if(!['mod','admin'].includes(rank))return res.status(400).json({error:'Invalid rank.'});const tokenCost=rank==='mod'?5:20,diamondCost=rank==='mod'?0:20;if((a.ownerTokens||0)<tokenCost)return res.status(400).json({error:`You need ${tokenCost} Owner Tokens.`});if((a.diamonds||0)<diamondCost)return res.status(400).json({error:`You need ${diamondCost} Diamonds.`});a.ownerTokens-=tokenCost;a.diamonds-=diamondCost;a.ownerRankLicenses=a.ownerRankLicenses||{mod:0,admin:0};a.ownerRankLicenses[rank]=(a.ownerRankLicenses[rank]||0)+1;save(db);res.json({ok:true,ownerTokens:a.ownerTokens,diamonds:a.diamonds,licenses:a.ownerRankLicenses})});
app.post('/api/server-owner/rankup',auth,(req,res)=>{const db=req.db,a=req.account,sid=String(req.body?.serverId||''),target=String(req.body?.target||a.username).toLowerCase(),rank=String(req.body?.rank||'mod');if(!isServerOwner(db,a,sid))return res.status(403).json({error:'Server Owner access required.'});if(!db.accounts[target]||!db.servers[sid]?.members.includes(target))return res.status(400).json({error:'Player must be in this server.'});if(!['mod','admin'].includes(rank))return res.status(400).json({error:'Invalid rank.'});a.ownerRankLicenses=a.ownerRankLicenses||{mod:0,admin:0};if((a.ownerRankLicenses[rank]||0)<1)return res.status(400).json({error:`You need a saved ${rank.toUpperCase()} rank license.`});a.ownerRankLicenses[rank]--;db.servers[sid].roles=db.servers[sid].roles||{};db.servers[sid].roles[target]=rank;save(db);res.json({ok:true,serverRole:rank,licenses:a.ownerRankLicenses})});
...
app.post('/api/owner/power',auth,(req,res)=>{if(!isRealOwner(req.account))return res.status(403).json({error:'Owner only.'});const db=req.db,action=String(req.body?.action||''),sid=String(req.body?.serverId||''),srv=db.servers[sid];if(['freeze','rename','clear'].includes(action)&&!srv)return res.status(404).json({error:'Server not found.'});if(action==='freeze'){srv.frozen=!srv.frozen}else if(action==='rename'){const n=String(req.body?.name||'').trim();if(!n)return res.status(400).json({error:'Enter a name.'});srv.name=n.slice(0,60)}else if(action==='clear'){for(const a of Object.values(db.accounts))a.messages=(a.messages||[]).filter(m=>m.server!==sid)}else if(action==='grantTime'){const t=String(req.body?.target||'').toLowerCase(),amt=Math.floor(Number(req.body?.amount||0));if(!db.accounts[t]||amt<1)return res.status(400).json({error:'Invalid target/amount.'});db.accounts[t].timeBalance=(db.accounts[t].timeBalance||0)+Math.min(10000,amt)}else if(action==='grantLicense'){const t=String(req.body?.target||'').toLowerCase(),rank=String(req.body?.rank||'mod');if(!db.accounts[t]||!['mod','admin'].includes(rank))return res.status(400).json({error:'Invalid target/rank.'});db.accounts[t].ownerRankLicenses=db.accounts[t].ownerRankLicenses||{mod:0,admin:0};db.accounts[t].ownerRankLicenses[rank]++}else return res.status(400).json({error:'Unknown owner power.'});save(db);res.json({ok:true,server:srv||null})});
app.post('/api/owner-shop/license',auth,(req,res)=>{
  if(!isRealOwner(req.account))return res.status(403).json({error:'Only the owner can purchase and grant these licenses.'});
  const db=req.db,a=req.account,target=String(req.body?.target||'').trim().toLowerCase(),rank=String(req.body?.rank||'mod').trim().toLowerCase(),serverId=String(req.body?.serverId||'').trim();
  const costs={mod:{tokens:5,diamonds:0,points:0},admin:{tokens:20,diamonds:20,points:0},serverAdmin:{tokens:30,diamonds:50,points:0},serverOwner:{tokens:25,diamonds:50,points:500}};
  const c=costs[rank];if(!c)return res.status(400).json({error:'Unknown owner license.'});if(!db.accounts[target])return res.status(404).json({error:'Player not found.'});
  if((a.ownerTokens||0)<c.tokens)return res.status(400).json({error:`You need ${c.tokens} Owner Tokens.`});if((a.diamonds||0)<c.diamonds)return res.status(400).json({error:`You need ${c.diamonds} Diamonds.`});if((a.points||0)<c.points)return res.status(400).json({error:`You need ${c.points} Points.`});
  if(rank==='serverAdmin'||rank==='serverOwner'){const srv=db.servers[serverId];if(!srv||srv.isUpdateLog)return res.status(400).json({error:'Choose a valid server.'});if(!srv.members.includes(target))srv.members.push(target);const ta=db.accounts[target];ta.servers=ta.servers||[];if(!ta.servers.includes(serverId))ta.servers.push(serverId);}
  a.ownerTokens-=c.tokens;a.diamonds-=c.diamonds;a.points-=c.points;const ta=db.accounts[target];
  if(rank==='mod')ta.role='mod';else if(rank==='admin')ta.role='admin';else if(rank==='serverAdmin'){ta.role='server-admin';ta.serverAdminServerId=serverId;ta.serverAdminExpiresAt=Date.now()+60*60*1000}else if(rank==='serverOwner'){ta.role='user';db.servers[serverId].owner=target;db.servers[serverId].roles=db.servers[serverId].roles||{};db.servers[serverId].roles[target]='server-owner'}
  save(db);res.json({ok:true,message:`${rank==='serverOwner'?'SERVER OWNER':rank.toUpperCase()} granted to ${ta.name}.`,account:clean(a),target:clean(ta)});
});
